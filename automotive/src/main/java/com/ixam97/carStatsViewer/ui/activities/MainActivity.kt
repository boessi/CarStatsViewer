...
class MainActivity : FragmentActivity() {
                ...
                CarStatsViewer.dataProcessor.realTimeDataFlow.collectLatest {
                    ...
                    if (it.isInitialized()) {
                        ...
                        with(binding){
                            ...
                            mainChargeSocGage.setValue((it.stateOfCharge!! * 100f).roundToInt())
                            ...
                            CarStatsViewer.dataProcessor.staticVehicleData.batteryCapacity?.let { batteryCapacity ->
                                ...
                                mainChargeSocGage.setValue((it.batteryLevel!! / (batteryCapacity) * 100).roundToInt())
                            }
    ...
    private fun  setGageAndPlotUnits(consumptionUnit: Boolean, distanceUnit: DistanceUnitEnum) = with(binding) {
        ...
        mainConsumptionPlot.dimensionRestrictionMin = appPreferences.distanceUnit.asUnit(DISTANCE_TRIP_DIVIDER)
        mainConsumptionPlot.dimensionRestrictionMax = appPreferences.distanceUnit.asUnit(DISTANCE_TRIP_DIVIDER) * 10
    ...
    private fun updateLocationStatusIcon(status: Int) = with(binding) {
        when(status) {
            ...
            WatchdogState.NOMINAL -> {
                ...
                mainIconLocationStatus.setColorFilter(getColor(R.color.connected_blue))
            ...
            WatchdogState.ERROR -> {
                ...
                mainIconLocationStatus.setColorFilter(getColor(R.color.bad_red))
                mainIconLocationStatus.visibility = View.VISIBLE
            }
            WatchdogState.LIMITED -> {
                mainIconLocationStatus.setImageDrawable(getDrawable(R.drawable.ic_location_error))
                mainIconLocationStatus.setColorFilter(getColor(R.color.limited_yellow))
                mainIconLocationStatus.visibility = View.VISIBLE
            }
        }
    }
    ...
    private fun setupDefaultUi() = with(binding) {
        ...
        mainConsumptionPlot.dimensionYPrimary = PlotDimensionY.CONSUMPTION
        ...
        mainConsumptionPlot.dimensionYSecondary = PlotDimensionY.IndexMap[appPreferences.secondaryConsumptionDimension] ?: PlotDimensionY.CONSUMPTION
        ...
        mainChargeSocGage.gageName = getString(R.string.main_gage_SoC)
        mainChargeSocGage.gageUnit = "%"
        mainChargeSocGage.primaryColor = getColor(R.color.charge_plot_color)
        mainChargeSocGage.minValue = 0f
        mainChargeSocGage.maxValue = 100f
        mainChargeSocGage.setValue(0f)
    }
